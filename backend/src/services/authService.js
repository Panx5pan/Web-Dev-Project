...
export default {
  async register({ username, password, profilePicture }) {
    ...
    const existing_user = db.users.find(u => u.username === username);
    if (existing_user) {
  ...
  async login({ username, password }) {
    ...
    const existing_user = db.users.find(u => u.username === username && u.password === password);
    ...
    const token = jwt.sign({ userId: existing_user.id, username: existing_user.username }, 
